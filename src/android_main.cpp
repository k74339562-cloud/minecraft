...
// -------------------------------------------------------------
// محرك فيزياء التصادم وصعود العتبات (AABB Collision & Step-Up)
// -------------------------------------------------------------
static bool isBoxColliding(Engine* e, glm::vec3 pos) {
    float r = 0.28f; // نصف قطر جسم اللاعب
        ...
        for (int z = minZ; z <= maxZ; ++z) {
            for (int y = minY; y <= maxY; ++y) {
                ...
                if (e->chunk->getBlock(x, y, z) != BlockType::Air) {
                    return true; // اصطدام ببلوكة صلبة
                }
            }
        }
    }
...
static void updatePlayer(Engine* e, float dt) {
    ...
    if (glm::length(moveDir) > 0.05f) {
        ...
        // 1. فحص الحركة الأفقية X مع صعود العتبة إذا كانت <= 1 بلوكة
        glm::vec3 testX = p.position + glm::vec3(moveDir.x * p.speed * dt, 0.0f, 0.0f);
        if (!isBoxColliding(e, testX)) {
            p.position.x = testX.x;
        } else if (p.isGrounded && !isBoxColliding(e, testX + glm::vec3(0, 1.05f, 0))) {
            p.position = testX + glm::vec3(0, 1.05f, 0); // صعود عتبة 1 بلوكة فقط
        }

        // 2. فحص الحركة الأفقية Z مع صعود العتبة إذا كانت <= 1 بلوكة
        glm::vec3 testZ = p.position + glm::vec3(0.0f, 0.0f, moveDir.z * p.speed * dt);
        if (!isBoxColliding(e, testZ)) {
            p.position.z = testZ.z;
        } else if (p.isGrounded && !isBoxColliding(e, testZ + glm::vec3(0, 1.05f, 0))) {
            p.position = testZ + glm::vec3(0, 1.05f, 0); // صعود عتبة 1 بلوكة فقط
        }
    }

    // 3. تطبيق الجاذبية والسقوط الحر
    p.velocity.y += p.gravity * dt;
    float nextY = p.position.y + p.velocity.y * dt;

    if (p.velocity.y < 0.0f) { // هبوط للأسفل
        glm::vec3 testDown = glm::vec3(p.position.x, nextY, p.position.z);
        if (isBoxColliding(e, testDown)) {
            p.position.y = std::ceil(nextY); // استقرار تام فوق سطح البلوكة
            p.velocity.y = 0.0f;
            p.isGrounded = true;
        } else {
            p.position.y = nextY;
    ...
    } else { // قفز للأعلى (اصطدام بالسقف إن وجد)
...
static int initDisplay(Engine* e) {
    ...
    // تفعيل حجب الأوجه الخلفية لمنع التداخل والـ Z-Fighting نهائياً!
    ...
    // بناء شجرة ماينكرافت واقعية بجذع يخترق الأوراق حتى القمة
    int ty = getTerrainHeight(8, 8) + 1;
    for (int y = 0; y < 4; ++y) e->chunk->setBlock(8, ty + y, 8, BlockType::Dirt);
    ...
    e->chunk->setBlock(7, ty + 4, 8, BlockType::OakLeaves);
    e->chunk->setBlock(9, ty + 4, 8, BlockType::OakLeaves);
    e->chunk->setBlock(8, ty + 4, 7, BlockType::OakLeaves);
    e->chunk->setBlock(8, ty + 4, 9, BlockType::OakLeaves);
