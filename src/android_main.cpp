...
struct Player {
    glm::vec3 position{8.0f, 66.0f, 8.0f}; // يبدأ اللاعب فوق مستوى البحر 63
...
static void initClouds(Engine* e) {
    ...
    constexpr int G = 24; constexpr float C = 4.0f, Y0 = 128.0f, Y1 = 132.0f, off = (G * C) * 0.5f; // الغيوم عند ارتفاع 128
    ...
    if (AInputEvent_getType(ev) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        if (actionMasked == AMOTION_EVENT_ACTION_DOWN || actionMasked == AMOTION_EVENT_ACTION_POINTER_DOWN) {
            ...
            if (glm::distance(glm::vec2(x, y), glm::vec2(jX, jY)) < 60.0f) {
                ...
                if (e->player.inWater) e->player.velocity.y = 4.0f; // سباحة للأعلى في الماء
    ...
    for (int x = minX; x <= maxX; ++x) {
        for (int z = minZ; z <= maxZ; ++z) {
            for (int y = minY; y <= maxY; ++y) {
                ...
                if (isBlockOpaque(b)) return true; // الماء لا يمنع الحركة كجدار صلب
            }
        }
    }
    return false;
...
static void updatePlayer(Engine* e, float dt) {
    ...
    // فحص هل اللاعب داخل الماء عند مستواه الحالي
    ...
    float currentGravity = p.inWater ? -4.0f : p.gravity; // طفو وسلاسة في الماء
...
static int initDisplay(Engine* e) {
    ...
    // ==============================================================
    // بناء عالم ماينكرافت الحقيقي من -64 إلى 320 مع بحار عند 63!
    // ==============================================================
    ...
    for (int x = 0; x < 16; ++x) {
        for (int z = 0; z < 16; ++z) {
            ...
            // 1. حجر الأساس عند قاع العالم -64
            e->worldColumn->setBlock(x, WORLD_MIN_Y, z, BlockType::Bedrock);

            // 2. طبقات الصخور العميقة من -63 حتى ما قبل السطح
            for (int y = WORLD_MIN_Y + 1; y < h - 3; ++y) {
                e->worldColumn->setBlock(x, y, z, BlockType::Stone);
            }

            // 3. طبقات الشواطئ والبحار أو التلال العشبية:
            if (h <= SEA_LEVEL + 1) { // منطقة شاطئ وبحر
                for (int y = std::max(WORLD_MIN_Y + 1, h - 3); y <= h; ++y) {
                    e->worldColumn->setBlock(x, y, z, BlockType::Sand); // رمال الشاطئ
                }
                // ملء الماء الشفاف حتى مستوى البحر الرسمي 63
                for (int y = h + 1; y <= SEA_LEVEL; ++y) {
                    e->worldColumn->setBlock(x, y, z, BlockType::Water);
                }
            } else { // تلال خضراء فوق مستوى البحر
                for (int y = h - 3; y < h; ++y) {
    ...
    // زراعة شجرة في قمة التل فوق مستوى البحر
    int tx = 11, tz = 11;
    ...
    // بناء المش لجميع المقاطع غير الفارغة
    e->worldColumn->buildAllMeshes();

    // رسبنة اللاعب فوق مستوى البحر على الشاطئ أو التل
...
static void drawFrame(Engine* e) {
    ...
    // 1. رسم مقاطع عمود العالم (24 Sub-Chunks)
    ...
    // تفعيل دمج الشفافية للماء
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

    for (int i = 0; i < NUM_SECTIONS; ++i) {
        if (e->worldColumn->sections[i] && !e->worldColumn->sections[i]->isEmpty()) {
            int secY = (i * SECTION_SIZE) + WORLD_MIN_Y;
            glm::mat4 mModel = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, (float)secY, 0.0f));
            glUniformMatrix4fv(glGetUniformLocation(e->blockShader, "uModel"), 1, GL_FALSE, &mModel[0][0]);
            e->worldColumn->sections[i]->render();
        }
    }
    glDisable(GL_BLEND);
    ...
    e->windOffset += 0.020f;
    if (e->windOffset > 4.0f * 24.0f) e->windOffset = 0.0f;
