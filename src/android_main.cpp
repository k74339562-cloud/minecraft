...
// -------------------------------------------------------------
// رسم الواجهة 2D (مع إيقاف CULL_FACE لتظهر الأزرار واضحة تماماً!)
// -------------------------------------------------------------
static void renderAllUI(Engine* e) {
    glDisable(GL_DEPTH_TEST);
    glDisable(GL_CULL_FACE); // حل مشكلة اختفاء الأزرار!
    glEnable(GL_BLEND);
    ...
    // 1. علامة التصويب في المنتصف (Crosshair +)
    ...
    glm::vec4 padActive(0.35f, 0.75f, 0.35f, 0.85f); // أخضر عند الضغط
    ...
    glEnable(GL_CULL_FACE); // إعادة تفعيل الحجب للثلاثي الأبعاد
...
// -------------------------------------------------------------
// حركة اللاعب والقفز التلقائي الطبيعي (بدون رسبنة!)
// -------------------------------------------------------------
static void updatePlayer(Engine* e, float dt) {
    ...
    if (glm::length(moveDir) > 0.05f) {
        ...
        if (!isBoxColliding(e, testX)) {
            ...
            // Auto-Jump طبيعي: إعطاء دفعة قفز حقيقية بدل الرسبنة الفورية!
...
static int initDisplay(Engine* e) {
    ...
    // ==============================================================
    // بناء شجرة ماينكرافت كثيفة وممتلئة بجذع صلب يخترق الأوراق
    // ==============================================================
    int tx = 8, tz = 8;
    int ty = getTerrainHeight(tx, tz) + 1;

    // 1. جذع الخشب يمتد حتى قمة الشجرة (يملأ القلب ويمنع الفراغ الداخلي)
    for (int y = 0; y < 5; ++y) {
        e->chunk->setBlock(tx, ty + y, tz, BlockType::Dirt);
    }

    // 2. طبقة أوراق عريضة 5x5
    for (int ox = -2; ox <= 2; ++ox) {
        for (int oz = -2; oz <= 2; ++oz) {
            if (std::abs(ox) == 2 && std::abs(oz) == 2) continue; // زوايا مشطوبة
            if (ox != 0 || oz != 0) {
...
static void drawFrame(Engine* e) {
    ...
    // 3. رسم واجهة أزرار اللمس (ستظهر واضحة جداً الآن!)
    renderAllUI(e);
