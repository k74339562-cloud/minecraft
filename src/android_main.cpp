...
static bool isBoxColliding(Engine* e, glm::vec3 pos) {
    ...
    for (int x = minX; x <= maxX; ++x) for (int z = minZ; z <= maxZ; ++z) for (int y = minY; y <= maxY; ++y) {
        ...
        if (isBlockOpaque(e->worldColumn->getBlock(x, y, z))) return true;
    }
    return false;
}
...
static int initDisplay(Engine* e) {
    ...
    // بناء العمود الكامل بارتفاع 384
    ...
    for (int x = 0; x < 16; ++x) for (int z = 0; z < 16; ++z) {
        ...
        for (int y = WORLD_MIN_Y + 1; y < h - 3; ++y) e->worldColumn->setBlock(x, y, z, BlockType::Stone);
        if (h <= SEA_LEVEL + 1) {
            for (int y = std::max(WORLD_MIN_Y + 1, h - 3); y <= h; ++y) e->worldColumn->setBlock(x, y, z, BlockType::Sand);
            for (int y = h + 1; y <= SEA_LEVEL; ++y) e->worldColumn->setBlock(x, y, z, BlockType::Water);
        } else {
            for (int y = h - 3; y < h; ++y) e->worldColumn->setBlock(x, y, z, BlockType::Dirt);
            e->worldColumn->setBlock(x, h, z, BlockType::Grass);
        }
    }

    int tx = 11, tz = 11, ty = getTerrainHeight(tx, tz) + 1;
    if (ty > SEA_LEVEL) {
        ...
        for (int ox = -2; ox <= 2; ++ox) for (int oz = -2; oz <= 2; ++oz) {
            ...
            if (ox != 0 || oz != 0) { e->worldColumn->setBlock(tx + ox, ty + 2, tz + oz, BlockType::OakLeaves); e->worldColumn->setBlock(tx + ox, ty + 3, tz + oz, BlockType::OakLeaves); }
        }
        for (int ox = -1; ox <= 1; ++ox) for (int oz = -1; oz <= 1; ++oz) if (ox != 0 || oz != 0) e->worldColumn->setBlock(tx + ox, ty + 4, tz + oz, BlockType::OakLeaves);
