...
// -------------------------------------------------------------
// شيدر الواجهة 2D (لأزرار اللمس والتصويب)
// -------------------------------------------------------------
...
struct Player {
    ...
    const float eyeHeight = 1.62f; // ارتفاع العين لماينكرافت
...
// -------------------------------------------------------------
// نظام اللمس المتقدم (Multi-Touch Input State)
// -------------------------------------------------------------
struct TouchInput {
    // حركة الـ D-Pad
    int movePointerId = -1;
    glm::vec2 moveInput{0.0f}; // X: Strafe, Y: Forward
...
// -------------------------------------------------------------
// رسم الواجهة المجمعة بالكامل (Single Draw Call UI)
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// نظام استقبال وتوزيع أحداث اللمس المتعدد (Multi-Touch)
// -------------------------------------------------------------
static int32_t handleInput(struct android_app* app, AInputEvent* ev) {
    ...
    if (AInputEvent_getType(ev) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        float dpadX = 180.0f;
        float dpadY = (float)e->height - 180.0f;
        float dpadRadius = 140.0f;

        float jX = (float)e->width - 160.0f;
        float jY = (float)e->height - 160.0f;
        float jRadius = 60.0f;

        // عند لمس الشاشة بإصبع جديد
        if (actionMasked == AMOTION_EVENT_ACTION_DOWN || actionMasked == AMOTION_EVENT_ACTION_POINTER_DOWN) {
            ...
            // 2. هل اللمس في منطقة الـ D-Pad (النصف الأيسر)؟
            if (x < (float)e->width * 0.45f) {
                ...
                if (glm::length(diff) > 10.0f) {
                    ...
                    e->touch.moveInput.y = -e->touch.moveInput.y; // عكس Y للأمام
                }
                return 1;
            }

            // 3. خلاف ذلك: الإصبع مخصص لتدوير الكاميرا (النصف الأيمن)
...
// -------------------------------------------------------------
// فيزياء وحركة اللاعب الكينماتيكية (Kinematic Voxel Physics)
// -------------------------------------------------------------
static void updatePlayer(Engine* e, float dt) {
    Player& p = e->player;

    // حساب متجهات اتجاه الكاميرا
    glm::vec3 front;
    ...
    // اتجاهات المشي المسطحة على الأرض X/Z
    ...
    if (e->chunk) {
        for (int y = 15; y >= 0; --y) {
            if (e->chunk->getBlock(bx, y, bz) != BlockType::Air) {
                groundHeight = (float)y + 1.0f; // سطح البلوكة
                break;
            }
        }
    }

    // استقرار اللاعب فوق سطح البلوكات
    ...
    // موضع الكاميرا = موقع أقدام اللاعب + ارتفاع العين
...
static void drawFrame(Engine* e) {
    ...
    // حساب Delta Time الحقيقي للفريمات
    auto now = std::chrono::steady_clock::now();
    float dt = std::chrono::duration<float>(now - e->lastFrameTime).count();
    e->lastFrameTime = now;
    if (dt > 0.05f) dt = 0.05f; // حماية ضد التقطيع المفاجئ

    // تحديث فيزياء وموقع اللاعب
    updatePlayer(e, dt);
    ...
    // 2. رسم الغيوم 3D (تتحرك مع الرياح فوق رأس اللاعب)
    ...
    // 3. رسم أزرار التحكم وعلامة التصويب (Touch UI)
    renderAllUI(e);
