...
// حساب ارتفاع سطح الأرض بمقاييس ماينكرافت الحقيقية (حول مستوى البحر 63)
inline int getTerrainHeight(int x, int z) {
    float n1 = smoothNoise(x * 0.05f, z * 0.05f) * 18.0f; // تلال عريضة
    float n2 = smoothNoise(x * 0.12f, z * 0.12f) * 6.0f;  // تفاصيل صخرية
    int h = 64 + (int)(n1 + n2); // تتراوح التضاريس بين 52 (بحار) إلى 88 (جبال)
    return h;
}
