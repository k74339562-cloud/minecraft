...
void main() {
    ...
    if (uUnderwater == 1) rgb = mix(rgb, vec3(0.05, 0.22, 0.55), 0.55); // شيدر تحت الماء
    ...
    if (AInputEvent_getType(ev) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        if (masked == AMOTION_EVENT_ACTION_DOWN || masked == AMOTION_EVENT_ACTION_POINTER_DOWN) {
            ...
            if (glm::distance(glm::vec2(x, y), glm::vec2(jX, jY)) < 60.0f) {
                ...
                if (e->player.inWater) e->player.velocity.y = 4.0f; // سباحة للأعلى
...
// تصادم كامل مع كل البلوكات الصلبة بما فيها أوراق الشجر والخشب!
...
static void updatePlayer(Engine* e, float dt) {
    ...
    // تحديث العالم العملاق وتوليد المقاطع الجديدة أثناء مشي اللاعب
...
static void drawFrame(Engine* e) {
    ...
    // فحص هل عين الكاميرا تحت الماء لتفعيل الشيدر ولون السماء المائي!
    ...
    if (eyeUnderwater) {
        glClearColor(0.04f, 0.18f, 0.45f, 1.0f); // سماء زرقاء بحرية داكنة
    } else {
        glClearColor(0.53f, 0.81f, 0.92f, 1.0f); // سماء صافية
    }
    ...
    // رسم كل الـ Chunks الممتدة حول اللاعب في العالم الشاسع
    for (auto& [key, col] : e->world->chunks) {
        for (int i = 0; i < NUM_SECTIONS; ++i) {
            if (col->sections[i] && !col->sections[i]->isEmpty()) {
                ...
                col->sections[i]->render();
            }
        }
    }
    glDisable(GL_BLEND);

    // رسم الغيوم ثلاثية الأبعاد
